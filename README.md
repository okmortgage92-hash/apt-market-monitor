...
## AI 리포트 (Google Gemini 무료 API)

"AI 리포트" 탭 → **리포트 생성** 버튼을 누르면, 코드가 계산한 확정 수치를
Gemini가 4개 섹션(시장 요약·지역별 동향·리스크 진단·종합 의견) 보고서로 서술합니다.
숫자 계산·판정은 전부 코드가 하고, AI는 문장만 씁니다.

- `api/report.js` = Gemini를 호출하는 심부름꾼. API 키를 숨긴 채 처리.
- Gemini 무료 등급이라 결제수단 없이 하루 일정 횟수까지 무료.
- 인쇄/PDF 버튼으로 보고서를 문서로 저장 가능.

### 무료 API 키 넣는 법
1. **aistudio.google.com/apikey** 접속 → Google 로그인 → **Create API key**
   (결제수단 등록 불필요, 무료 등급)
2. Vercel → 프로젝트 → Settings → Environments → Production →
...
> ⚠️ 이 기능은 공개 부동산원 데이터를 외부(Google)로 보내 문장을 받아옵니다.
> 공개 시세라 민감정보는 아니지만, 사내 외부 API 사용 규정을 먼저 확인하세요.
> 실고객 데이터로 확장할 때는 반드시 사내 on-prem LLM으로 대체해야 합니다.
>
> 사내 LLM(Alli) 등 다른 모델로 바꾸려면 `api/report.js`의 fetch 주소와
> 요청 형식만 교체하면 됩니다. 대시보드 쪽은 수정 불필요.

---

## 제목·문구 바꾸는 법

화면의 모든 글자는 `src/App.jsx` 안에 있습니다.
1. GitHub 저장소 → `src/App.jsx` → 연필(Edit) 아이콘
2. 바꿀 글자 수정 (예: "아파트 시장 모니터링" → 원하는 제목)
3. 맨 아래 Commit changes
4. → Vercel이 자동으로 다시 배포 (Redeploy 불필요, 1~2분 뒤 반영)

주요 문구 위치:
- 대시보드 제목/부제 → App() 함수 상단 헤더 부분
- 각 탭 이름 → `const tabs = [...]`
- 카드 제목 → 각 CardHead의 title
